//! Vendored MIT `minisign-verify` 0.2.5 (Frank Denis) as a path module.
//! TEST ONLY usage for Keysmith Switch updater policy tests.
//!
...
use self::crypto::blake2b::{Blake2b, BLAKE2B_OUTBYTES};
...
pub fn decode_base64(input: &str) -> Result<Vec<u8>, Error> {
    Base64::decode_to_vec(input.trim()).map_err(|_| Error::InvalidEncoding)
}
