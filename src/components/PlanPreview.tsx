...
export function PlanPreview({ envelope }: { envelope: Envelope }) {
  ...
  return (
    <div className="space-y-3 text-[14px]" data-testid="plan-preview">
      {drifted ? (
        <div className="flex items-start gap-2 rounded-2xl border border-amber-600/40 bg-amber-500/10 px-3 py-2 text-amber-700 dark:text-amber-400">
      ...
      {!gate.ok ? (
        <div className="flex items-start gap-2 rounded-2xl border border-destructive/40 bg-destructive/10 px-3 py-2 text-destructive">
          <IconAlert size={14} className="mt-px shrink-0" />
      ...
      {envelope.error ? (
        <p className="rounded-2xl bg-destructive/10 px-3 py-2 text-destructive">{envelope.error}</p>
      ) : null}
